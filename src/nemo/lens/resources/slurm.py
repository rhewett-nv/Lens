...
SLURM_INTEGER_RESOURCE_ATTRIBUTE_KEYS = frozenset(
    {
        SLURM_ARRAY_COUNT,
        SLURM_NNODES,
        SLURM_NTASKS,
...
def _select_slurm_resource_attributes(
    ...
    for key, value in attrs.items():
        if key not in allowed or key in retired:
            continue
        if key in SLURM_INTEGER_RESOURCE_ATTRIBUTE_KEYS:
            _set_int(selected, key, value)
        else:

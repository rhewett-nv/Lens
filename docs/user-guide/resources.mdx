...
the local environment. It also preserves selected non-empty Slurm and run
identity values from launch-provided `OTEL_RESOURCE_ATTRIBUTES`; those launch
values take precedence over caller-provided `resource_attributes` defaults and
